...
{
    if(!_storkeColor){
        _storkeColor = [UIColor darkGrayColor];
    }
    return _storkeColor;
}
...
{
    //context behaviors
    CGContextSetLineWidth(context, rect.size.width/45);
   ...
   //                                        rectForPicture.origin.y + rectForPicture.size.height);
    ...
    pointOne = CGPointMake(center.x - 2*thingWidth, cornerRightBottom.y + thingWidth);
    ...
    pointFour = CGPointMake(center.x + 2*thingWidth, cornerRightBottom.y + thingWidth);
